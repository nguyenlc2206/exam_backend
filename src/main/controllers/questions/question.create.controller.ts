...
export class CreateQuestionController {
    ...
    execute = catchAsync(
        async (req: HttpRequestUser, res: Response, next: NextFunction) => {
            ...
            const checkIdExamResult = await this.handleCheckIdExam(
                req.body.examId
            );
            ...
            const saveQuestionsResult = await this.handleCreateQuestions(
                req.body
            );
    ...
    /** @todo: check id exam exists */
    private handleCheckIdExam = async (
        id: string
    ): Promise<Either<ExamsEntity | undefined, AppError>> => {
        const exam = await this._examService.getById(id);
        if (!exam)
            return failure(
                new AppError("ExamId is not exists in database!", 400)
            );
        return success(exam);
    };

    /** @todo: save data to table questions_entity */
    private handleCreateQuestions = async (
        body: any
