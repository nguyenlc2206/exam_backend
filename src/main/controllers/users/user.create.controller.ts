...
export class CreateUserController {
    ...
    execute = catchAsync(
        async (req: Request, res: Response, next: NextFunction) => {
            ...
            const checkEmailResult = await this.hanleCheckEmailExists(
                req.body.email
            );
            ...
            if (checkEmailResult.data?.deletedAt) {
                ...
                const restoreResult = await this.handleRestoreUser(
                    checkEmailResult.data.id
                );
    ...
    /** @todo: check email exists */
    private hanleCheckEmailExists = async (
        email: string
    ): Promise<Either<UsersEntity | undefined, AppError>> => {
        const userFinded = await this._userService.getUserByEmail(email);
        if (userFinded && !userFinded?.deletedAt)
            return failure(new AppError("Email is already in database!", 400));
    ...
    /** @todo: restore user to database */
    private handleRestoreUser = async (
        id: string
    ): Promise<Either<any, AppError>> => {
