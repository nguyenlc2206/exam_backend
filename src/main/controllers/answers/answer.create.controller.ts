...
export class CreateAnwserController {
    ...
    execute = catchAsync(
        async (req: Request, res: Response, next: NextFunction) => {
            ...
            const checkListIdResult = this.handleCheckListIds(
                req,
                listIds.data
            );
            ...
            const saveAnswersCorrectResult =
                await this.handleUpdateAnswerCorrect(saveAnswersResult.data);
    ...
    /** @todo: get all list id questions in database */
    private handleGetAllQuestionsId = async (): Promise<
        Either<Array<string>, AppError>
    > => {
    ...
    /** @todo: check list id question of body */
    private handleCheckListIds = (
        req: HttpRequest,
        listIds: Array<string>
    ): Either<boolean, AppError> => {
        ...
        const hasAllElems = listIdsData.every((elem: any) =>
            listIds.includes(elem)
        );
        if (!hasAllElems)
            return failure(
                new AppError("Something wrong list questions ids!", 400)
            );
        return success(true);
    };

    /** @todo: get all answers by question id */
    private handleGetAnswersByQuestionId = async (
        id: string
    ): Promise<Either<AnswersEntity[], AppError>> => {
        const answers = await this._answerService.getByQuestionId(id);
        return success(answers);
    };

    /** @todo: delete answers exists */
    private handleDeleteAnswers = async (
        item: AnswersEntity[]
    ): Promise<void> => {
    ...
    /** @todo: save data to table answers_entity */
    private handleCreateAnswers = async (
        req: Request
    ): Promise<Either<AnswersEntity[], AppError>> => {
        ...
        Object.values(req.body.data).map(async (question: any) => {
            ...
            /** @todo: get all answers by question id */
            const listAnswersExists = await this.handleGetAnswersByQuestionId(
                question?.questionId
            );
            if (listAnswersExists.isFailure())
                return failure(listAnswersExists.error);
