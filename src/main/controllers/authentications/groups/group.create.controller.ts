...
export class CreateGroupController {
    ...
    execute = catchAsync(
        async (req: Request, res: Response, next: NextFunction) => {
            ...
            const checkNameGroupError = await this.handleCheckName(
                req.body.name
            );
    ...
    private handleCheckName = async (
        name: string
    ): Promise<Either<GroupsEntity | undefined, AppError>> => {
        const urlFinded = await this._groupsServices.getByName(name);
        if (urlFinded)
            return failure(new AppError(`Name ${name} is already!`, 400));
    ...
    private handleCreateGroup = async (
        item: GroupsEntity
    ): Promise<Either<GroupsEntity, AppError>> => {
