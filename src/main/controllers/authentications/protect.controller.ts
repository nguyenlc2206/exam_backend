...
export class ProtectRoutesController {
    ...
    execute = catchAsync(
        async (req: HttpRequestUser, res: Response, next: NextFunction) => {
            ...
            const verifyTokenResult = await this.handleVerifyToken(
                _accessToken
            );
            ...
            const getUserByEmail = await this.handleGetUserByEmail(
                keyParse?.email
            );
            ...
            const checkPasswordChangedAfter = this.handlePasswordChangedAfter(
                _iat,
                _passwordChangedAt
            );
    ...
    /** @todo: Getting tokken and check of it's there**/
    private handleGetTokenFromHeaders = (
        req: HttpRequest
    ): Either<string, AppError> => {
        let accessToken;
        if (
    ...
    /** @todo: Verification token **/
    private handleVerifyToken = async (
        token: string
    ): Promise<Either<DecodeAccountTokenType, AppError>> => {
