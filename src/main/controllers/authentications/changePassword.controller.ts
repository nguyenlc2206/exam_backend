...
import { HttpRequestUser, HttpRequest, UserInformation } from '@src/shared/entities/http.entity';
import { Validation, Either, success } from '@src/shared/functions';
...
export class ChangePasswordController {
    ...
    execute = catchAsync(async (req: HttpRequestUser, res: Response, next: NextFunction) => {
        ...
        if (checkPasswordCorrect.isFailure()) return next(checkPasswordCorrect.error);
        if (!checkPasswordCorrect.data) return next(new AppError('Current password is invalid!', 400));
        ...
        if (changePasswordResult.isFailure()) return next(changePasswordResult.error);
    ...
    private handleValidation = (request: HttpRequest) => {
        ...
        validations.push(new CompareFieldsValidation('newPassword', 'passwordConfirm'));
