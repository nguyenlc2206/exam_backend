...
export class GetAllExamsWithUserIdController {
    ...
    execute = catchAsync(
        async (req: HttpRequestUser, res: Response, next: NextFunction) => {
            ...
            const listExamsResult = await this.handleGetAllExamsByUserId(
                req.userInfor.userId
            );
            if (listExamsResult.isFailure()) return next(listExamsResult.error);

            /** @todo: process data to response */
            const resDataProc = await this.handleProcessResponse(
                listExamsResult.data
            );
