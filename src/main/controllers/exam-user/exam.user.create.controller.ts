...
export class CreateExamRelationUserController {
    ...
    execute = catchAsync(
        async (req: Request, res: Response, next: NextFunction) => {
            ...
            const checkIdUserResult = await this.handleCheckIdUser(
                req.body.userId
            );
            ...
            const hasAllElems = req.body.examId.every((elem: string) =>
                listIds.includes(elem)
            );
            if (!hasAllElems)
                return next(
                    new AppError("Something wrong from list exam ids!", 400)
                );
    ...
    /** @todo: check id user exists */
    private handleCheckIdUser = async (
        id: string
    ): Promise<Either<UsersEntity, AppError>> => {
        const user = await this._userService.getUserById(id);
        if (!user)
            return failure(
                new AppError("User is not exists in database!", 400)
            );
        return success(user);
    };

    /** @todo: check id exam exists */
    private handleGetAllExamId = async (): Promise<
    ...
    private handleCreateExamUser = async (
        item: ExamUserEntity[]
