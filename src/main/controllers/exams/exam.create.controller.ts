...
export class CreateExamController {
    ...
    execute = catchAsync(
        async (req: HttpRequestUser, res: Response, next: NextFunction) => {
            ...
            const getIdCategory = await this.handleGetIdCategory(
                req.body.category
            );
    ...
    /** @todo: get category id with name category */
    private handleGetIdCategory = async (
        name: string
    ): Promise<Either<string, AppError>> => {
        const itemCategory = await this._categoryServices.getCategoryByName(
            name
        );
        if (!itemCategory)
            return failure(new AppError("Name category not exists!", 400));
        return success(itemCategory?.id);
    };

    /** @todo: save exams to database */
    private handleCreateExam = async (
        item: ExamsEntity
    ): Promise<Either<ExamsEntity, AppError>> => {
